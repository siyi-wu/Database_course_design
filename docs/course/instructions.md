...
`sql/01_schema/01_schema_and_seed.sql` 建立了 5 张核心表：
...
`sql/02_programmability/01_cursor_procedures.sql` 已经实现 2 个使用游标的存储过程，满足题目 `(4)`：
...
`sql/03_indexes/01_index_analysis.sql` 已建立多个二级索引：
...
项目继续完成题目数据库要求中的 `(2)(3)(6)`：

- `(2)` 至少 3 个触发器。
- `(3)` 至少 2 个带参存储过程或函数。
- `(6)` 使用规范化理论建立模式的详细过程。

此外，题目还要求系统需求分析文档、ER 模型、系统设计说明书、程序设计代码、测试报告和操作手册，相关材料统一放入 `docs/` 目录。
...
计划实现 2 个带参存储过程和 1 个带参函数，其中前两个已经足以满足题目 `(3)`，函数用于增强演示展示。
...
虽然 `docs/design/02_normalization_design_part_1_4_5.md` 已经包含规范化说明，但题目 `(6)` 仍需要提供更详细、更系统的规范化文档。该文档应包括：
...
1. 不重新执行 `sql/01_schema/01_schema_and_seed.sql`，除非明确需要重置全库。
2. 先确认基础表、游标过程和索引已经存在。
3. 执行 `sql/02_programmability/02_triggers.sql` 创建触发器。
4. 执行 `sql/02_programmability/03_parameterized_routines.sql` 创建带参过程和函数。
5. 执行 `sql/05_demo/01_demo_and_test.sql` 做演示测试。
...
- `sql/01_schema/01_schema_and_seed.sql`：建表、外键和初始化数据。
- `sql/02_programmability/01_cursor_procedures.sql`：游标存储过程。
- `sql/02_programmability/02_triggers.sql`：3 个触发器。
- `sql/02_programmability/03_parameterized_routines.sql`：带参存储过程和函数。
- `sql/03_indexes/01_index_analysis.sql`：索引设计和查询分析。
- `sql/04_benchmark/`：benchmark 数据和查询脚本。
- `sql/05_demo/01_demo_and_test.sql`：演示和测试脚本。

`docs/` 目录用于整理课程设计文档：

- `docs/analysis/`：系统需求分析和 ER 模型。
- `docs/design/`：系统设计说明书和规范化过程。
- `docs/manual/`：操作手册。
- `docs/testing/`：测试报告、运行结果和性能测试记录。
...
1. 说明系统主题：实验室设备管理系统。
2. 展示数据表、外键、游标过程和索引性能分析。
3. 展示触发器、带参过程/函数、规范化分析。
...
7. 打开 `docs/design/03_normalization_process_part_2_3_6.md`，说明从未规范化表到 3NF 的过程。
8. 使用 `docs/` 目录中的需求分析、ER 模型、系统设计、操作手册和测试报告支撑完整大作业文档。
